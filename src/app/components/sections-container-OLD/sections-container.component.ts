...
export class SectionsContainerComponent implements OnDestroy {
    ...
    protected navColor = signal('#000000');
    ...
    private updateNavColor(index: number) {
        if (!isPlatformBrowser(this.platformId)) return;

        const section = this.sections[index] as HTMLElement;
        if (!section) return;

        // Use setTimeout to ensure styles are applied if transitioning
        setTimeout(() => {
            const style = window.getComputedStyle(section);
            const bg = style.backgroundColor; // rgb(r, g, b)
            ...
            if (rgb && rgb.length >= 3) {
                ...
                // If dark (< 128), use white text. Else black.
                this.navColor.set(brightness < 128 ? '#ffffff' : '#000000');
            }
        }, 50);
    }
}
