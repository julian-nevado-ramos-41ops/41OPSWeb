...
@Component({
  ...
  template: `
    <div class="left-indicator">
      @if (totalSections() > 0 && !isAccordionSection() && !fullWidth()) {
        ...
        <div class="text-container">
          ...
          @if (modalContent()) {
            <button class="see-more-btn" (click)="openModal($event)">
              <span class="see-more-text">{{ t().common.seeMore }}</span>
    ...
    <div class="right-nav">
      @if (totalSections() > 0 && !isAccordionSection() && !fullWidth()) {
        <app-side-nav [totalSections]="totalSections()" [currentSection]="globalCurrentSection()" [style.--nav-color]="navColor()" (sectionClicked)="navigate.emit($event)" />
  ...
  styles: [`
    ...
    @media (max-width: 900px) {
      ...
      .left-indicator {
        ...
        left: 1.5rem;
        width: auto;
        height: auto; /* Fix: Override the base height: 100% so it doesn't push content off the top */
        z-index: 100;
        pointer-events: auto;
      }
